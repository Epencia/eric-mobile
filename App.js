...
import Aurore from './screens/aurore';
import Gestionnaire from './screens/gestionnaire';
...
export default function App() {
  // Ajouter l'état Aurore & Gestionnaire & Inscription
const [auroreModalVisible, setAuroreModalVisible] = useState(false);
const [gestionnaireModalVisible, setGestionnaireModalVisible] = useState(false);
const [inscriptionModalVisible, setInscriptionModalVisible] = useState(false);
  ...
  // Inscription
  const handleInscriptionSuccess = (login) => {
  setLoginInput(login);
      ...
      <SafeAreaView style={styles.container} edges={["top", "bottom"]}>
        ...
        <KeyboardAvoidingView style={styles.loginContainer} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
          ...
          <View style={styles.loginForm}>
            ...
            <TouchableOpacity style={styles.inscriptionLink} onPress={() => setInscriptionModalVisible(true)}>
  <Text style={styles.inscriptionLinkText}>
    Pas encore de compte ? <Text style={styles.inscriptionLinkHighlight}>S'inscrire</Text>
  </Text>
</TouchableOpacity>
          </View>
          <Text style={styles.loginFooter}>🔐 Connexion sécurisée - Epencia</Text>
        </KeyboardAvoidingView>
        {renderForgotModal()}
        <Inscription 
  visible={inscriptionModalVisible}
  onClose={() => setInscriptionModalVisible(false)}
      ...
      <SafeAreaView style={styles.container} edges={["top", "bottom"]}>
        ...
        <View style={styles.header}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            ...
            <TouchableOpacity onPress={() => setAuroreModalVisible(true)} style={[styles.logoutButtonSmall, { marginRight: 4, backgroundColor: '#dbeafe' }]}>
...
>
  <Text style={styles.logoutTextSmall}>🗄️</Text>
        ...
        {/* ✅ MODAL AURORE - ICI AUSSI */}
      <Aurore 
        user={user}
        visible={auroreModalVisible}
        onClose={() => setAuroreModalVisible(false)}
      />
      <Gestionnaire 
  user={user}
...
const styles = StyleSheet.create({
  ...
  // Inscription
inscriptionLink: {alignItems: 'center',paddingVertical: 10,marginTop: 4,},
inscriptionLinkText: {fontSize: 14,color: '#6b7280',},
inscriptionLinkHighlight: {color: '#6c63ff',fontWeight: '600',},
});